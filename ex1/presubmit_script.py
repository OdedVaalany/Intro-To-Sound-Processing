...
def test_classify_digit_stream(root):
    ...
    assert isinstance(digit_stream, list) and len(digit_stream) > 0 and isinstance(
        digit_stream[0], int), "return type should be list of ints"
...
def test_naive_time_stretch_temporal(root):
    ...
    assert isinstance(
        stretched_wav, torch.Tensor), "return type should be a torch tensor"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root', type=str,
                        default=str(Path(os.path.abspath(__file__)).parent))
    ...
    try:
        ...
        raise Exception(
            "Invalid ID file, should contain a single line following: <id>,<id> format")

    assert os.path.exists(
        f"{cur_dir}/digit_classifier.py"), "Missing digit_classifier.py file"
    assert os.path.exists(
        f"{cur_dir}/general_utilities.py"), "Missing general_utilities.py file"
    assert os.path.exists(
        f"{cur_dir}/time_stretch.py"), "Missing time_stretch.py file"

    # ---- Utils ----
    ret = load_wav(f"{root}/audio_files/Basta_16k.wav")
    assert len(ret) == 2, "load_wav should return (wav, sr) tuple"
    assert isinstance(ret[0], torch.Tensor) and ret[0].dtype == torch.float32 and isinstance(
        ret[1], int), "load_wav should return (torch.Tensor (float), int)"

    dummy = torch.randn(1, 32000)
    stft_res = do_stft(dummy, n_fft=512)
    assert isinstance(
        stft_res, torch.Tensor) and stft_res.shape[-1] == 2, "do_stft should return torch.Tensor with expected shape"

    fft_res = do_fft(dummy)
    assert isinstance(fft_res, torch.Tensor) and fft_res.dtype in {
        torch.complex32, torch.complex64}, "do_stft should return torch.Tensor with expected type"

    ret = do_istft(stft_res, n_fft=512)
    assert isinstance(
        ret, torch.Tensor) and ret.shape[-1] == 32000, "do_istft should return torch.Tensor with expected shapes"
